...
const ProfilePage = () => {
  ...
  const [bio,setBio]=useState("Hi Everyone, I am using ChatSphere");
      ...
      <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2
       border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
        <form onSubmit={handleSubmit} action="" className='flex flex-col gap-5 p-10 flex-1'>
          ...
          <input type="text" onChange={(e)=>setName(e.target.value)} value={name} required placeholder='Your name' className='p-2 border border-gray-500 rounded-md focus-within:outline-none focus:ring-2 focus:ring-violet-500' />
          <textarea name="" id="" placeholder="Write profile bio" required className='p-2 border border-gray-500 rounded-md focus-within:outline-none focus:ring-2 focus:ring-violet-500' rows={4}></textarea>
