  ...
  return (
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      ...
      {currState=="Sign up" && !isDataSubmitted && (
         ...
         required className='p-2 border border-gray-500 rounded-md focus:outline-noone focus:ring-2 focus:ring-indigo-500'/>
      )}
      {!isDataSubmitted && (
        <>
        <input onChange={(e)=>setEmail(e.target.value)} type="email" 
        value={email} placeholder='Email Address' required 
        className='p-2 border border-gray-500 rounded-md focus:outline-noone focus:ring-2 focus:ring-indigo-500'/>
      ...
      {currState=="Sign up" && isDataSubmitted && (
        ...
        rw={4} className='p-2 border border-gray-500 rounded-md
       ...
       <div className='flex flex-col gap-2'>
        {currState ==="Sign up"? (
          ...
          <p className='text-sm text-gray-600'>Create an account <span onClick={()=>
            setCurrState("Sign up")} className='font-medium text-violet-500 cursor-pointer'
            >Click here</span></p>
        )
      }
       </div>
